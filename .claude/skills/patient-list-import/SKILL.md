...
- **檢驗值後面的 `*` 會被丟掉**，只有 `H` / `L` 會保留。院內系統的 `*` 觀察下來是血氣
  那一列的註腳符號，不是異常標記（pH 7.362 正常也帶 `*`），丟掉是對的。
- **不用自己補 `H` / `L`**。App 從 v1.8.0 起會依參考值自動判讀並標色，v1.9.0 起超過
  危險值還會標 `HH` / `LL`，所以匯入檔照原始報告寫就好；只有報告單自己印了旗標才跟著
  寫進去（那會蓋過自動判讀，`HH` / `LL` 也認）。
  **單位一定要填對**，這是自動判讀準不準的關鍵——單位對不上 App 就整格不判讀。
