/* Patient List — service worker
 *
 * 目的：讓 App 外殼（HTML/JS/圖示）離線也開得起來。
 *
 * 同一個 github.io 網域底下還有其他 PWA（Clinical-Tools、題庫、待辦…），
 * 所以這裡嚴格自我約束：
 *   - 快取名稱一律用 PREFIX 開頭，清理時只刪自己的，絕不動別人的
 *   - 只處理本 scope（/patient-list/）底下的同源 GET
 *   - Supabase 的 API 一律不碰，永遠走網路（也絕不快取任何病人資料）
