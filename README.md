...
- **Lab data**：欄＝時間、列＝檢驗項目的表格，過寬時整塊橫向捲動、第一欄固定；可貼上檢驗報告文字自動帶入。表頭的時間只顯示月／日與時間（年份省略），欄寬預設不設限，讓表格自己撐滿版面；只有在**手機上整張表塞不進畫面時**才啟用第一欄限寬（檢驗項目抓「plasmacell %」剛好放得下的 116px），超過截斷成 `…` 並可左右滑。電腦瀏覽一律不限寬。完整名稱與完整時間都放在 title 裡。
